...
# Scale close and mentor columns
scaled_close = scaler_close.fit_transform(df['close'].values.reshape(-1, 1))
scaled_optimal = scaler_optimal.fit_transform(df['mentor'].values.reshape(-1, 1))
...
def train_model():
    ...
    # Save the model
    save_path = '../gru_weights/gru_mt.pth'
...
model_load_path = '../gru_weights/gru_mt.pth'
model = GRUModel(input_size, hidden_size, output_size, num_layers).to(device)
...
# Adjust the length of actual mentor values to align with predicted values
actual_optimal = df['mentor']
...
# Plotting actual vs. predicted mentor values
